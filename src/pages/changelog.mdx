...
- The Nix package build skips its check phase. The check re-ran the whole test suite inside the Nix sandbox on a 7 GB ARM runner, which the kernel OOM killer ended partway through the test binary on every single run; the same tests already run in the Test workflow on runners with room to breathe, so the Nix job goes back to answering the question it exists for — does the package build ([#1828](https://github.com/raphamorim/rio/pull/1828), by [@bet4it](https://github.com/bet4it)).
