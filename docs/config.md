...
- `trail-cursor-color`: Trail color as a hex string (e.g. `"#F07178"`). Unset, the trail takes the cursor color. Note: while a trail is in flight it passes over the cursor cell, so a custom color briefly tints the cursor during the animation.
